...
def get_out_df(df_sig,df_coeff,key_cols):
  out_dict = {"gene" : [], "variable" : [], "bool" : [], "sig" : [], "coeff" : []}
  bool_dict = {"False" : False, "True" : True, "FALSE" : False, "TRUE" : True}
  for gene in df_sig.index:
    sig = df_sig.loc[gene,:][~df_sig.loc[gene,:].isna()]
    coeff = df_coeff.loc[gene,:][~df_coeff.loc[gene,:].isna()]
    for i, sig_val in sig.iteritems():
      if i in key_cols:
        for j, coeff_val in coeff.iteritems():
          if j.startswith(i):
            out_dict["gene"].append(gene)
            out_dict["variable"].append(i)
            out_dict["bool"].append(bool_dict[j[len(i):]])
...
def main():
  ...
  #  dn = "TS_10x_redo"
    ...
    df = get_out_df(df_sig, df_coeff,key_cols)
    for i in range(len(args.all_datanames)):
      for j in range(i + 1, len(args.all_datanames)):
        ...
        for dataname in datanames:
          ...
          out_df = get_out_df(df_sig, df_coeff,key_cols)
          out_df["gene_variable"] = out_df["gene"] + out_df["variable"] + out_df["bool"].astype(str)
